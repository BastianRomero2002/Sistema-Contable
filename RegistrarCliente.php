...
$consulta=$cn->prepare('SELECT * FROM persona_activa WHERE codigo_persona NOT IN (SELECT codigo_persona FROM trabajador) AND codigo_persona NOT IN (SELECT codigo_persona FROM cliente)');
$consulta->execute();
?>


<fieldset>
<legend><strong>Informacion Cliente</strong></legend>
<table width="776" height="70" border="0" cellpadding="2" cellspacing="0">
 ...
 <tr>
     ...
     <td scope="row"><label for="label3">Gusto:</label></td>
     <td><input type="text" id="Gusto"/> 
 ...
 <tr>
     <td height="22" scope="row">&nbsp;</td>
     <td scope="row"><label for="label3">Preferencia:</label></td>
     <td><input type="text" id="Preferencia"/> 
...
<table width="790"  border="0" id="example">
  <thead>
    <tr >
      ...
      <td width="125"><span class="Estilo15">Gusto</span></td>
	        <td width="125"><span class="Estilo15">Preferencia</span></td>
    </tr>
  </thead>
  <tfoot>
    <tr>
...
include('Conexion.php');
	$sql=$cn->prepare('SELECT * FROM cliente');	
	$sql->execute();		
...
<tr>
  ...
  <td><span class="Estilo3"><?php echo $op['gusto'];?></span></td>
   <td><span class="Estilo3"><?php echo $op['preferencia'];?></span></td>
