...
$consulta=$cn->prepare('SELECT * FROM producto WHERE codigo_producto NOT IN (SELECT codigo_producto FROM almacenado_en)');
$consulta->execute();
...
<fieldset>
<legend><strong>Informacion Almacenado En</strong></legend>
<table width="776" height="74" border="0" cellpadding="2" cellspacing="0">
          
 <tr>
   <td width="26" scope="row">&nbsp;</td>
   <td width="205" height="26" scope="row"><label for="label2">Codigo Producto:</label></td>
   <td width="213">
   
    <select size="1" name="Genero" id="CodigoProducto">
	...
	?>
             ...
             <option value="<?php echo $result['codigo_producto']?>"><?php echo $result['nombre'];?></option>
...
<table width="788"  border="0" id="example">
  <thead>
    <tr >
	  <td width="128" height="25"><span class="Estilo15">Codigo Producto</span></td>
 ...
 <td width="128" height="25"><span class="Estilo15">Codigo Producto</span></td>
  ...
  <tbody>
	...
	$sql=$cn->prepare('SELECT * FROM almacenado_en');	
	$sql->execute();		
