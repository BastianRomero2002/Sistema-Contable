...
$consulta=$cn->prepare('SELECT * FROM cliente WHERE codigo_persona NOT IN (SELECT codigo_persona FROM carrito)');
$consulta->execute();
?>
...
<fieldset>
<legend><strong>Informacion Carrito</strong></legend>
<table width="776" height="74" border="0" cellpadding="2" cellspacing="0">
          
 <tr>
   <td width="12" scope="row">&nbsp;</td>
   <td width="169" height="26" scope="row"><label for="label2">Codigo Persona:</label></td>
   <td width="275">
 ...
 <tr>
	 ...
	 <option value="Carrito de Compras Universal">Carrito de Compras Universal</option>
...
<div style="margin:10px auto; width:790px;">

<table width="783"  border="0" id="example">
  <thead>
    <tr >
  ...
  <tbody>
	...
	$sql=$cn->prepare('SELECT * FROM carrito');	
	$sql->execute();		
